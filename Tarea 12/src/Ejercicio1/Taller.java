        ...
        if (garantia.equalsIgnoreCase("s")) {
            ...
            do {
                try {
                    ...
                    Ordenador_Garantia og = new Ordenador_Garantia(cod_rep, cod_cli, descripcion,
                            fecha_compra);
                    ordenadores.add(og);
                    okparse = true;
                } catch (Exception e) {
    ...
    public static boolean introPresupuesto(String cod_reparacion) {
        for (Ordenador ordenador : ordenadores) {
            if (cod_reparacion.equalsIgnoreCase(ordenador.getCod_rep()) && ordenador.getEstado() == 0) {
                if (!(ordenador instanceof Ordenador_Garantia)) {
                    Scanner sc = new Scanner(System.in);
    ...
    public static boolean aceptPresupuesto(String cod_reparacion) {
        ...
        for (Ordenador ordenador : ordenadores) {
            if (cod_reparacion.equalsIgnoreCase(ordenador.getCod_rep()) && ordenador.getEstado() == 0) {
                if (!(ordenador instanceof Ordenador_Garantia)) {
                    System.out.println(
                            "Presupuesto: " + ordenador.getPresupuesto() + " ¿Acepta usted el presupuesto? (s/n)");
                    respuesta = sc.nextLine();
                    if (respuesta.equalsIgnoreCase("s")) {
                        ordenador.setEstado(1);
                    }
                } else
                    ordenador.setEstado(2);// Se puede hacer la retirada del ordenador
